 ...
 */
class tx_weeaargooglesitemap_module1 extends \TYPO3\CMS\Backend\Module\BaseScriptClass {
	...
	function main() {
		...
		if (($this->id && $access) || ($BE_USER->user['admin'] && !$this->id)) {
			...
			$this->content .= '<div style="margin-left: 30px;">';
	...
	function saveSubmit() {
		$priority = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('priority');
		$period = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('period');
		$site = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('site');
	...
	function moduleContent() {
		...
		switch ((string) $this->MOD_SETTINGS['function']) {
			case 1:
				...
				if (\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('priority')) {
					$this->saveSubmit();
				}
				$this->getData();
	...
	function getPageInfo($pages, $level = 0) {
		...
		$img_path = '../' . \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath('weeaar_googlesitemap') . 'mod1/';
		...
		for ($i = 1; $i < count($pages); $i++) {
			...
			if ($pages[($i + 1)]['level'] != $pages[$i]['level'] && $pages[($i + 1)]['level'] >= $startLevel) {
				for ($j = 2; $j < $pages[$i]['level']; $j++) {
					$content .= "<img src='" . $img_path . "img/sub1.gif'>";
				}
				$content .= "<img src='" . $img_path . "img/sub.gif'>";
			} else {
	...
	function getPagetreeOrder($siteRootUid) {
		...
		// l0:
		$l0 = 0;
		while ($l0 < count($pages)) {
			if ($pages[$l0]['uid'] == $startPid) {
				$curL0Uid = $pages[$l0]['uid'];
				$order[] = array('uid' => $curL0Uid, 'level' => '1', 'title' => $pages[$l0]['title'], 'doktype' => $pages[$l0]['doktype']);
				//l1:
				$l1 = 0;
				while ($l1 < count($pages)) {
					if ($pages[$l1]['pid'] == $curL0Uid) {
						$curL1Uid = $pages[$l1]['uid'];
						$order[] = array('uid' => $curL1Uid, 'level' => '2', 'title' => $pages[$l1]['title'], 'doktype' => $pages[$l1]['doktype']);
						//l2:
						$l2 = 0;
						while ($l2 < count($pages)) {
							if ($pages[$l2]['pid'] == $curL1Uid) {
								$curL2Uid = $pages[$l2]['uid'];
								$order[] = array('uid' => $curL2Uid, 'level' => '3', 'title' => $pages[$l2]['title'], 'doktype' => $pages[$l2]['doktype']);
								//l3:
								$l3 = 0;
								while ($l3 < count($pages)) {
									if ($pages[$l3]['pid'] == $curL2Uid) {
										$curL3Uid = $pages[$l3]['uid'];
										$order[] = array('uid' => $curL3Uid, 'level' => '4', 'title' => $pages[$l3]['title'], 'doktype' => $pages[$l3]['doktype']);
										//l4:
										$l4 = 0;
										while ($l4 < count($pages)) {
											if ($pages[$l4]['pid'] == $curL3Uid) {
												$curL4Uid = $pages[$l4]['uid'];
												$order[] = array('uid' => $curL4Uid, 'level' => '5', 'title' => $pages[$l4]['title'], 'doktype' => $pages[$l4]['doktype']);
												//l5:
												$l5 = 0;
												while ($l5 < count($pages)) {
													if ($pages[$l5]['pid'] == $curL4Uid) {
														$curL5Uid = $pages[$l5]['uid'];
														$order[] = array('uid' => $curL5Uid, 'level' => '6', 'title' => $pages[$l5]['title'], 'doktype' => $pages[$l5]['doktype']);
													}
													$l5++;
												} // end l5
											}
					...
					switch((string)$this->MOD_SETTINGS['function'])	{
						case 1:
							$content='<div align=center><strong>Hello World!</strong></div><br />
								...
								'GET:'.\TYPO3\CMS\Core\Utility\GeneralUtility::view_array($_GET).'<br />'.
								'POST:'.\TYPO3\CMS\Core\Utility\GeneralUtility::view_array($_POST).'<br />'.
								'';
...
// Make instance:
//$SOBE = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\tx_weeaargooglesitemap_module1');
$SOBE = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_weeaargooglesitemap_module1');
$SOBE->init();
